...
class BackgroundViewController: UIViewController {
    ...
    private var instructionBGLabel = UILabel().then {
        $0.text = "피팅된 자신의 모습을 올려주세요"
    ...
    private lazy var uploadBGButton = UIButton().then {
    ...
    private var navigationbarTitle = UILabel().then {
        $0.text = "피팅사진 업로드"
        $0.textColor = .darkGray
    }
    ...
    private func render() {
        ...
        view.addSubview(instructionBGLabel)

        view.addSubview(backgroundImageView)
        //profileImageView.addSubview(imageDimView)
        view.addSubview(clearButton)

        view.addSubview(uploadBGButton)

        instructionBGLabel.snp.makeConstraints {
        ...
        backgroundImageView.snp.makeConstraints {
            $0.top.equalTo(instructionBGLabel.snp.bottom).inset(-16)
...
//        }
        ...
        uploadBGButton.snp.makeConstraints {
...
extension BackgroundViewController {
    ...
    private func uploadButtonClciked() {
        print("피팅된 사진 업로드")
        provider.request(Link.postBackroundImage(image: (backgroundImageView.image!))) { response in
            switch response {
            case .success(let result):
                do {
                    if result.statusCode == 200 {
                        print("========배경사진 업로드 성공 시 받는 데이터============")
...
extension BackgroundViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        self.backgroundImageView.image = newImage // 받아온 이미지를 update
